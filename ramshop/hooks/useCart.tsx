...
export const CartContextProvider = (props: Props) => {
  ...
  useEffect(() => {
    const cartItems: any = localStorage.getItem("shopCartItem");
  ...
  const handleAddProductToCart = useCallback((product: CartProductType) => {
    setCartProducts((prev) => {
      ...
      localStorage.setItem("shopCartItem", JSON.stringify(updatedCart));
  ...
  const handleRemoveProductFromCart = useCallback(
    (product: CartProductType) => {
      if (cartProducts) {
        ...
        localStorage.setItem("shopCartItem", JSON.stringify(filtredProducts));
  ...
  const handleClearCart = useCallback(() => {
    ...
    setCartTotalQty(0);
    localStorage.setItem("shopCartItem", JSON.stringify(null));
    toast.success("Cart Cleared with success");
  }, [cartProducts]);
